<div class="row">
  <div class="col-md-3">
    <div class="card card-info">
      ...
      <form action="{$base_url}worker/insert" method="post">
        ...
        <div class="card-body">
          ...
          <div class="input-group mb-3">
            <input type="text" class="form-control" id="worker_id" name="worker_id" required maxlength="20" placeholder="Mã nhà máy" {literal}pattern="[A-Za-z0-9-_]{3,}" {/literal}>
          </div>
          <div class="input-group mb-3">
            <input type="text" class="form-control" id="factory_id" name="factory_id" placeholder="Mã công nhân" maxlength="20" {literal}pattern="[A-Za-z0-9-_]{3,}" {/literal}>
          </div>
          <div class="input-group mb-3">
          ...
          <div class="form-group">
            <div class="input-group">
              ...
              <select class="form-control" name="congdoan_id" id="congdoan_id">
                ...
                <option value="{$cd->congdoan_id}" {if $congdoan_id eq $cd->congdoan_id}selected{/if}>{$cd->congdoan_name}</option>
                {/foreach}
              </select>
            </div>
          </div>
    ...
    <div class="card card-info">
      ...
      <form action="{$base_url}worker/import" method="post" enctype="multipart/form-data">
        <input type="hidden" name="group_id" value="{$group->group_id}">
        <div class="card-body">
          <div class="form-group">
            <div class="input-group">
            ...
            <div class="input-group">
              <a href="{$base_url}worker.xls">Tải mẫu</a>
            </div>
          </div>
          <!-- /input-group -->
          <div class="form-group">
  ...
  <div class="col-md-9">
    <div class="card card-info">
      <div class="card-header">
        <h3 class="card-title">
          Danh sách công nhân {$group->group_name}
        </h3>
      </div>
      <div class="card-body">
          ...
          <table class="table table-hover table-bordered" id="dataTable" width="100%" cellspacing="0">
            ...
            <tbody>
              ...
              <tr align="center">
                ...
                <td>{$wk->note}</td>
                <td>
                  <a href="{$base_url}worker/card/{$wk->worker_id}">
                  ...
                  <a style="color: #da251d" href="{$base_url}worker/delete/{$wk->worker_id}/{$wk->group_id}" onclick="return confirm('Bạn có chắc chắn xóa không?');">
...
<div class="modal fade" id="update{$wk->worker_id}">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form class="form-horizontal" method="post" name="updateproduct" action="{$base_url}worker/update">
        ...
        <div class="modal-body">
          <div class="form-group row">
            ...
            <div class="col-sm-10">
              <input type="text" class="form-control" placeholder="Mã nhà máy" id="NEW_worker_id" name="new_worker_id" required maxlength="20" value="{$wk->worker_id}" {literal}pattern="[A-Za-z0-9+-_]{3,}" {/literal}>
            </div>
          </div>
          <div class="form-group row">
            <label for="factory_id" class="col-sm-2 col-form-label">Mã công nhân:</label>
            <div class="col-sm-10">
              <input type="text" class="form-control" placeholder="Mã công nhân" id="factory_id" name="factory_id" maxlength="20" value="{$wk->factory_id}" {literal}pattern="[A-Za-z0-9+-_]{3,}" {/literal}>
            </div>
          </div>
          <div class="form-group row">
          ...
          <div class="form-group">
            <div class="input-group">
              ...
              <select class="form-control" name="congdoan_id" id="congdoan_id">
                <option value="">Không</option>
                {foreach $congdoan as $cd}
                <option value="{$cd->congdoan_id}" {if $wk->congdoan_id eq $cd->congdoan_id}selected{/if}>{$cd->congdoan_name}</option>
                {/foreach}
              </select>
            </div>
          </div>
          <div class="form-group row">
            ...
            <div class="col-sm-10">
              <select class="form-control group_product" style="width: 100%;" name="group_id" id="group_id">
                {foreach $groups as $g}
                <option value="{$g->group_id}" {if $g->group_id eq $wk->group_id}selected{/if}>{$g->group_name}</option>
                {/foreach}
              </select>
            </div>
          </div>
