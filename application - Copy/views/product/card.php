    ...
    <div class="card card-info">
      ...
      <div class="card-body">
        ...
        {foreach $product as $pr}
          <option value="{$pr->product_id}" {if $pr->product_id eq $product_id} selected {/if}>({$pr->product_id}) {$pr->product_name}</option>
        {/foreach}
        </select>
      </div><!--card-body-->
    </div><!--card-info-->
...
<script>
  function _delete(card_id){
      ...
      if (x){
          $.post("{$base_url}product/card_delete",{
              card_id: card_id
  ...
  function loadlistcard(){
    ...
    $.post("{$base_url}product/card_read",
        {
    ...
    $( "#card_id" ).keyup(function(event){
      if(event.keyCode == 13){
        ...
        if (($.isNumeric($("#card_id" ).val()))&&($("#card_id" ).val().length==10)){
          $.post("{$base_url}product/card_create",
          {
            card_id: $( "#card_id" ).val(),
