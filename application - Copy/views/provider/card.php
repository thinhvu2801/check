    ...
    <div class="card card-info">
      ...
      <div class="card-body">
        ...
        {foreach $provider as $pr}
          <option value="{$pr->provider_id}" {if $pr->provider_id eq $provider_id} selected {/if}>({$pr->provider_id}) {$pr->provider_name}</option>
        {/foreach}
        </select>
      </div><!--card-body-->
    </div><!--card-info-->
...
<script>
  function _delete(card_id){
      ...
      if (x){
          $.post("{$base_url}provider/card_delete",{
              card_id: card_id
  ...
  function loadlistcard(){
    ...
    $.post("{$base_url}provider/card_read",
        {
    ...
    $("#card_id").keyup(function(event){
      if(event.keyCode == 13){
        ...
        if (($.isNumeric($("#card_id" ).val()))&&($("#card_id" ).val().length==10)){
          $.post("{$base_url}provider/card_create",
          {
            card_id: $( "#card_id" ).val(),
