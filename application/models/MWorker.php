...
class MWorker extends CI_Model{
	...
	public function ReadDetail($table)
	{
	...
	public function RFIDInsert($worker_id,$factory_id)
    {
        $proc = "EXEC [dbo].[RFID_INSERT] @worker_id = '$worker_id' ,@factory_id = '$factory_id'";
        $query = $this->db->query($proc);
        return $query->result_array();
    }
	public function RFIDUpdate($worker_id,$factory_id)
    {
        $proc = "EXEC [dbo].[RFID_UPDATE] @worker_id = '$worker_id' ,@factory_id = '$factory_id'";
        $query = $this->db->query($proc);
        return $query->result_array();
    }
	...
	public function RFIDDelete($object_id)
    {
        $this->db->where('object_id', $object_id);
		$this->db->delete('RFID');
    }
	public function RFIDDeleteNghi($worker_id)
	{
		$this->db->where('object_id', $worker_id);
		$this->db->delete('RFID');
	}
