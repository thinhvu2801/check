    ...
    <div class="card card-info">
      ...
      <div class="card-body">
        ...
        {foreach $customer as $pr}
          <option value="{$pr->customer_id}" {if $pr->customer_id eq $customer_id} selected {/if}>({$pr->customer_id}) {$pr->customer_name}</option>
        {/foreach}
        </select>
      </div><!--card-body-->
    </div><!--card-info-->
...
<script>
  function _delete(card_id){
      ...
      if (x){
          $.post("{$base_url}customer/card_delete",{
              card_id: card_id
  ...
  function loadlistcard(){
    ...
    $.post("{$base_url}customer/card_read",
        {
    ...
    $("#card_id").keyup(function(event){
      if(event.keyCode == 13){
        ...
        if (($.isNumeric($("#card_id" ).val()))&&($("#card_id" ).val().length==10)){
          $.post("{$base_url}customer/card_create",
          {
            card_id: $( "#card_id" ).val(),
