  ...
  <div class="col-md-7">
    <div class="card card-info">
      <div class="card-header">
        <h3 class="card-title">Danh sách hầm đông</h3>
      </div><!--card-header-->
      <div class="card-body">
        ...
        {foreach $hamdong as $h}
          <option value="{$h->ham_id}" {if $h->ham_id eq $ham_id} selected {/if}>({$h->ham_id}) {$h->ham_name}</option>
        {/foreach}
        </select>
      </div><!--card-body-->
    </div><!--card-info-->
...
<script>
  function _delete(card_id){
      ...
      if (x){
          $.post("{$base_url}hamdong/card_delete",{
              card_id: card_id
  ...
  function loadlistcard(){
    ...
    $.post("{$base_url}hamdong/card_read",
        {
    ...
    $("#card_id").keyup(function(event){
      if(event.keyCode == 13){
        ...
        if (($.isNumeric($("#card_id" ).val()))&&($("#card_id" ).val().length==10)){
          $.post("{$base_url}hamdong/card_create",
          {
            card_id: $( "#card_id" ).val(),
