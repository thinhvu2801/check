  ...
  <div class="col-md-7">
    <div class="card card-info">
      <div class="card-header">
        <h3 class="card-title">Danh sách quy trình</h3>
      </div><!--card-header-->
      <div class="card-body">
        ...
        {foreach $process as $pr}
          <option value="{$pr->process_id}" {if $pr->process_id eq $process_id} selected {/if}>({$pr->process_id}) {$pr->process_name}</option>
        {/foreach}
        </select>
      </div><!--card-body-->
    </div><!--card-info-->
...
<script>
  function _delete(card_id){
      ...
      if (x){
          $.post("{$base_url}process/card_delete",{
              card_id: card_id
  ...
  function loadlistcard(){
    ...
    $.post("{$base_url}process/card_read",
        {
    ...
    $("#card_id").keyup(function(event){
      if(event.keyCode == 13){
        ...
        if (($.isNumeric($("#card_id" ).val()))&&($("#card_id" ).val().length==10)){
          $.post("{$base_url}process/card_create",
          {
            card_id: $( "#card_id" ).val(),
