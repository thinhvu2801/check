  ...
  <div class="col-md-7">
    <div class="card card-info">
      ...
      <div class="card-body">
        ...
        {foreach $khothe as $bk}
        <option value="{$bk->id}" {if $bk->id eq $id} selected {/if}>{$bk->id}</option>
        {/foreach}
      </select>
      </div><!--card-body-->
    </div><!--card-info-->
...
<script>
  function _delete(card_id){
      ...
      if (x){
          $.post("{$base_url}khothe/card_delete",{
              card_id: card_id
  ...
  function loadlistcard(){
    ...
    $.post("{$base_url}khothe/card_read",
        {
    ...
    $( "#card_id" ).keyup(function(event){
      if(event.keyCode == 13){
        ...
        if (($.isNumeric($("#card_id" ).val()))&&($("#card_id" ).val().length==10)){
          $.post("{$base_url}khothe/card_create",
          {
            card_id: $( "#card_id" ).val(),
