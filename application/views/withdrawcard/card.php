  ...
  <div class="col-md-5">
    <div class="card card-info">
      ...
      <div class="card-body">
        ...
        <option value="WITHDRAW" selected>Thẻ thu hồi</option>
      </select>
      </div><!--card-body-->
    </div><!--card-info-->
  </div><!--col-md5-->
...
<script>
  function _delete(card_id){
      ...
      if (x){
          $.post("{$base_url}withdrawcard/card_delete",{
              card_id: card_id
  ...
  function loadlistcard(){
    ...
    $.post("{$base_url}withdrawcard/card_read",
        {
    ...
    $( "#card_id" ).keyup(function(event){
      if(event.keyCode == 13){
        ...
        if (($.isNumeric($("#card_id" ).val()))&&($("#card_id" ).val().length==10)){
          $.post("{$base_url}withdrawcard/card_create",
          {
            card_id: $( "#card_id" ).val(),
