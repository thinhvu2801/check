...
class Worker extends CI_Controller
{
    ...
    public function View($group_id, $mes = "")
    {
        ...
        $data = array(
            ...
            'data' => array(
                ...
                'factory' => $this->MWorker->ReadDetail('RFID_MaThe'),
    ...
    public function View_cn_nghi($mes = "")
    {
        $this->load->model("MWorker");
        $this->load->model('MGroup');
        $data = array(
            'action' => 'worker_nghi',
            'template' => 'worker/index_cn_nghi',
            'data' => array(
                'groups' => $this->MGroup->Read(),
                'worker' => $this->MWorker->ReadDetail('Worker'),
                'factory' => $this->MWorker->ReadDetail('RFID_MaThe'),
                'mes' => $mes
            )
        );
    ...
    public function Insert()
    {
        if (isset($_POST["worker_id"])) {
            $this->load->model('MWorker');
            $this->load->model('MGroup');
            if(!preg_match('/^[a-zA-Z0-9-_]+$/', $_POST["worker_id"])) {
                $this->View($_POST["group_id"],"Mã công nhân không hợp lệ"); 
            }
            elseif ($this->MWorker->CheckCode($_POST["worker_id"]))
                ...
                $data = array(
                    ...
                    'congdoan_id' => $_POST["congdoan_id"],//dành cho làm theo nhóm (ADD-xẻ bướm)
                    'status' => true
                );
                $this->MWorker->Insert($data);
                $this->MWorker->RFIDInsert($_POST["worker_id"],$_POST["factory_id"]);
    ...
    public function Insert_nghi()
    {
        if (isset($_POST["worker_id"])) {
            $this->load->model('MWorker');
            $this->load->model('MGroup');
            $worker_id = $_POST["worker_id"];
            if(!preg_match('/^[a-zA-Z0-9-_]+$/', $worker_id)) {
            ...
            elseif ($this->MWorker->CheckCode($_POST["worker_id"]))
                ...
                redirect(base_url() . "worker/view_cn_nghi");
            }
        } else {
            redirect(base_url() . 'worker/view_cn_nghi');
        }
    ...
    public function Import()
    {
        ...
        if (isset($_POST['import'])) {
            if ($_FILES["file_xls"]["size"] > 0) {
                ...
                $file = fopen($fname, "r");
                $factory_ids = $this->MWorker->ReadDetail('RFID_MaThe');
                $factory_id_list = array_column($factory_ids, 'id');
                ...
                if ($emapData != array('stt', 'factory_id', 'worker_id', 'worker_name')) {
                    ...
                    while (($emapData = fgetcsv($file, 0, ",")) !== FALSE) {
                        $worker_id = trim($emapData[2]);
                        $factory_id = trim($emapData[1]);

                        // if (!in_array($factory_id, $factory_id_list)) {
                        //     $invalid_factory_ids[] = $factory_id;
                        // } else {
                            if ($worker_id != "") {
                                $data = array(
                                ...
                                $this->MWorker->RFIDInsert($emapData[2],$emapData[1]);
                                $status = "DONE";
                                if ($this->MWorker->CheckCode($emapData[2])) {
                                    $status = "W_ERROR";
                                    if (isset($_POST['thaythe'])) {
                                        $this->db->where('worker_id', $emapData[2]);
                                        $this->db->update('worker', $data);
                                        $mes = "Đã được thay thế bằng dữ liệu mới!";
                                    }
                                }
                                if ($status == "DONE") $this->MWorker->Insert($data);
                                else $wcode = $wcode . $emapData[2] . " ";
                            }
                        // }
                    }
                    ...
                    if (!empty($invalid_factory_ids)) {
                        $message = "Các mã thẻ: " . implode(', ', $invalid_factory_ids)." chưa có trong hệ thống";
                        $this->View($_POST["group_id"], $message);
                    } elseif (($wcode != "")) {
    ...
    public function Import_nghi()
    {
        ...
        if (isset($_POST['import'])) {
            if ($_FILES["file_xls"]["size"] > 0) {
                ...
                $emapData = fgetcsv($file, 0, ",");
                if ($emapData != array('stt', 'worker_id', 'worker_name', 'group_id'))
                    $this->View_cn_nghi( "Cấu trúc file dữ liệu không đúng!");
                else {
                    ...
                    while (($emapData = fgetcsv($file, 0, ",")) !== FALSE) {
                        ...
                        if ($worker_id != "") {
                            $data = array(
                                'worker_id' => $emapData[1],
                                
                                'worker_name' => $emapData[2],
                                'group_id' => $emapData[3],
                                'status' => false
                            );
                            $status = "DONE";
                    ...
                    unset($fname);
                    if (($wcode != ""))
                        $this->View_cn_nghi( "Các mã công nhân: " . $wcode . "đã có trong hệ thống! " . $mes);
                    else $this->View_cn_nghi();
                }
            } else {
                //redirect(base_url());
                echo "E1";
        ...
        if (isset($_POST["new_worker_id"])) {
            ...
            if ($_POST["new_worker_id"] != $_POST["old_worker_id"]) {
                if ($this->MWorker->CheckCode($_POST["new_worker_id"]))
                    ...
                    $this->MWorker->RFIDUpdate($_POST["new_worker_id"],$_POST["factory_id"]);
                    // $this->MWorker->RFIDDelete($wid);
                    // $this->MWorker->RFIDInsert();
    ...
    public function Update_nghi()
    {
        if (isset($_POST["new_worker_id"])) {
            ...
            if ($_POST["new_worker_id"] != $_POST["old_worker_id"]) {
                if ($this->MWorker->CheckCode($_POST["new_worker_id"]))
                    $this->View_cn_nghi("Mã công nhân đã tồn tại!");
                else {
                    $data = array(
                        'worker_id' => $_POST["new_worker_id"],
                        'factory_id' => $_POST["factory_id"],
...
//chuyển công nhân đang làm thành nghỉ làm
    public function Update_cn($worker_id, $current_status, $group_id)
    {
        if (!$this->session->userdata("username"))  redirect('user/login');
        $this->load->model('MWorker');
        ...
        $this->MWorker->Update($data, $worker_id);
        $this->MWorker->RFIDDeleteNghi($worker_id);
        redirect(base_url() . "worker/index/" . $group_id);
    }
//chuyển công nhân từ nghỉ làm sang làm lại
    public function Update_cn_nghi($worker_id, $current_status)
    {
        if (!$this->session->userdata("username"))  redirect('user/login');
        $this->load->model('MWorker');
        $new_status = ($current_status == 1) ? 0 : 1;
    ...
    public function Delete_nghi($wid)
    {
        if (!$this->session->userdata("username")) redirect('user/login');
        if (!$this->session->userdata("admin"))
            if (!in_array($this->object, $this->session->userdata("role")))
    ...
    public function WorkerForView()
    {
        ...
        $this->load->view('worker_forview',$data);
    }
    public function dataworkerforview()
    {
        ...
        $this->load->model("MCard");
        $result = $this->MCard->Check($card_id);
        $worker = $result[0]->object_id;
        // echo $worker;
        if(strpos($worker, 'DH') !== false)
        {
        ...
        else
        {
            ...
            $result_today = $this->MFillet->ForWorkerView($card_id, 1);
            $result_yesterday = $this->MFillet->ForWorkerView($card_id, 0);
            // echo $worker;
        }
        ...
        $this->view->parse('worker_dataview', $data);
    }
}
