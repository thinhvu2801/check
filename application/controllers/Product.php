...
class Product extends CI_Controller
{
    ...
    public function Import()
    {
        ...
        if (isset($_POST['import'])) {
            if ($_FILES["file_xls"]["size"] > 0) {
                ...
                $emapData = fgetcsv($file, 0, ",");
                if ($emapData != array('product_id', 'product_name', 'note','parent_id'))
                    ...
                    unset($fname);
                    if ($pcode != "")
                        $this->View("Các mã sản phẩm: " . $pcode . "Đã có trong hệ thống! " . $mes);
                    else $this->View();
                }
            } else {
                redirect(base_url());
    ...
    public function card($product_id = "0")
    {
        ...
        $data = array(
            ...
            'data' => array('product' => $product, 'product_id' => $product_id,'typecard' => $typecard)
        );
        $this->load->view('home', $data);
    }
    // public function card($basket_id = "")
    // {
    //     $typecard = $this->session->userdata('typecard');
    //     if(empty($typecard)){
    //         $file_name = "typecard.txt";
    ...
    public function dongbo(){
        ...
        $this->load->model("MProduct");
        $table_name = 'Product';
        $sel=$this->MProduct->Read1($table_name);
        $this->load->database();
        ...
        $this->MProduct->Update_Kho($insert,$table_name);
        return $this->View();
    }
}
