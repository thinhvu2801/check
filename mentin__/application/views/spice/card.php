  ...
  <div class="col-md-7">
    <div class="card card-info">
      ...
      <div class="card-body">
        ...
        {foreach $spice as $bk}
        <option value="{$bk->spice_id}" {if $bk->spice_id eq $spice_id} selected {/if}>({$bk->spice_id}) {$bk->spice_name}</option>
        {/foreach}
      </select>
      </div><!--card-body-->
    </div><!--card-info-->
...
<script>
  function _delete(card_id){
      ...
      if (x){
          $.post("{$base_url}spice/card_delete",{
              card_id: card_id
    ...
    $( "#card_id" ).keyup(function(event){
      if(event.keyCode == 13){
        ...
        if (($.isNumeric($("#card_id" ).val()))&&($("#card_id" ).val().length==10)){
          $.post("{$base_url}spice/card_create",
          {
            card_id: $( "#card_id" ).val(),
