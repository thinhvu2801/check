<div class="row">
  <div class="col-md-5">
    <div class="card card-info">
      <div class="card-header">
        <h3 class="card-title">Danh sách loại SP</h3>
      </div><!--card-header-->
      <div class="card-body">
        ...
        {foreach $product_type as $h}
          <option value="{$h->product_type_id}" {if $h->product_type_id eq $product_type_id} selected {/if}>({$h->product_type_id}) {$h->product_type_name}</option>
        {/foreach}
        </select>
      </div><!--card-body-->
    </div><!--card-info-->
  ...
  <div class="col-md-2">
    <div class="card card-info">
      <div class="card-header">
        <h3 class="card-title">Vào/ra</h3>
      </div><!--card-header-->
      <div class="card-body">
        <select size="20" class="form-control" name="in_out" id="in_out" style="font-size: 1.5em">
          <option value="+" selected>+ (Vào)</option>
...
<script>
  function _delete(card_id){
      ...
      if (x){
          $.post("{$base_url}producttype/card_delete",{
              card_id: card_id
  ...
  function loadlistcard(){
    ...
    $.post("{$base_url}producttype/card_read",
        {
          object_id: obj_id + $( "#in_out" ).val()//.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g, '')
        },
        function(data,status){
          $( "#listcard" ).html(data);
        }
    ...
    $("#card_id").keyup(function(event){
      if(event.keyCode == 13){
        ...
        if (($.isNumeric($("#card_id" ).val()))&&($("#card_id" ).val().length==10)){
          $.post("{$base_url}producttype/card_create",
          {
            card_id: $( "#card_id" ).val(),
            object_id: $( "#object_id" ).val() + $( "#in_out" ).val()
          },
          function(data,status){
            loadlistcard();
            $("#card_id").val("");
