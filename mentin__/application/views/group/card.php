  ...
  <div class="col-md-7">
    <div class="card card-info">
      ...
      <div class="card-body">
        ...
        {foreach $group as $gr}
          <option value="{$gr->group_id}" {if $gr->group_id eq $group_id} selected {/if}>({$gr->group_id}) {$gr->group_name}</option>
        {/foreach}
        </select>
      </div><!--card-body-->
    </div><!--card-info-->
...
<script>
  function _delete(card_id){
      ...
      if (x){
          $.post("{$base_url}group/card_delete",{
              card_id: card_id
    ...
    $( "#card_id" ).keyup(function(event){
      if(event.keyCode == 13){
        ...
        if (($.isNumeric($("#card_id" ).val()))&&($("#card_id" ).val().length==10)){
          $.post("{$base_url}group/card_create",
          {
            card_id: $( "#card_id" ).val(),
